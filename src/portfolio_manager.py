...
# ── Sizing parameters ────────────────────────────────────────────────────────
# MAX_SINGLE_POSITION and KELLY_FRACTION are now read from config/risk_params.yaml
# via _load_risk_params() → do not hardcode here.
...
def _compute_sizing(
    ...
    max_single_pos = float(params.get("max_position_pct", 0.15)) * 100.0
    sizing = round(min(sizing, max_single_pos), 1)
...
def _enrich_with_trade_levels(
    ...
    params = _load_risk_params()
    max_single_pos_pct = float(params.get("max_position_pct", 0.15)) * 100.0
    max_position_usd = PORTFOLIO_SIZE_USD * max_single_pos_pct / 100.0
