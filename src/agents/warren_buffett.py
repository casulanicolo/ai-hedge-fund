...
def calculate_owner_earnings(financial_line_items: list) -> dict[str, any]:
    ...
    if len(financial_line_items) >= 2:
        try:
            ...
            ca_prev = getattr(previous,  'current_assets',      None)
            cl_prev = getattr(previous,  'current_liabilities', None)
...
def estimate_maintenance_capex(financial_line_items: list) -> float:
    ...
    latest_depreciation = financial_line_items[0].depreciation_and_amortization or 0
    ...
    if len(capex_ratios) >= 3:
        ...
        latest_revenue  = getattr(financial_line_items[0], 'revenue', 0) or 0
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    details = []
    historical_earnings = [item.net_income for item in financial_line_items[:5] if getattr(item, 'net_income', None)]
    ...
    if len(historical_earnings) >= 3:
        ...
        if oldest > 0:
            hist_growth        = ((latest / oldest) ** (1 / years)) - 1
            hist_growth        = max(-0.05, min(hist_growth, 0.15))
            conservative_growth = hist_growth * 0.7
        else:
    ...
    details.append(f"Three-stage DCF: S1 {stage1_growth:.1%}, S2 {stage2_growth:.1%}, terminal {terminal_growth:.1%}")
...
def analyze_book_value_growth(financial_line_items: list) -> dict[str, any]:
    ...
    book_values = [
        ...
        if getattr(item, 'shareholders_equity', None) and getattr(item, 'outstanding_shares', None)
    ]
...
def analyze_pricing_power(financial_line_items: list, metrics: list) -> dict[str, any]:
    ...
    gross_margins = [
        item.gross_margin for item in financial_line_items
...
def generate_buffett_output(
    ...
    """Get investment decision from LLM with Buffett's philosophy."""
    ...
    template = ChatPromptTemplate.from_messages([
        (
            ...
            "\n"
            "Decision rules:\n"
            "- direction=LONG:    strong business AND margin_of_safety > 0\n"
            "- direction=SHORT:   poor business OR clearly overvalued\n"
            "- direction=NEUTRAL: mixed evidence or margin_of_safety <= 0\n"
            "\n"
            "expected_return: your estimated upside vs fair value over a 72h horizon "
            "(e.g. 0.03 = +3%). Positive for LONG, negative for SHORT, ~0 for NEUTRAL. "
            "Range: -0.10 to +0.10. Be realistic and conservative.\n"
            "\n"
            "confidence: 0.1 to 1.0 reflecting how clear-cut the signal is.\n"
            "\n"
